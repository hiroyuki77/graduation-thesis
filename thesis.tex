...
\end{figure}
\end{itemize}
　これらのクラッシュが起こった状況から見られるように，一部のブロックが使えなくなる状況が発生したり， garbage data をユーザに返してしまう状況が発生したりする．クラッシュが発生し，復帰したあとでも， file system に一貫性が保たれている状態にしたい．一方で， disk に対して一度に一つの write しか commit できないという制限がある．その制限の中で，クラッシュ後も file system が一貫性を保った状態で復帰できるようにするのが， journaling である．\\
...
　先ほどのクラッシュが起こった際の例で扱った簡単な書き込みを使って， metadata journaling の手順を説明する．最初に，実際に書き込みを行う前に journal に書き込まれるものは，図\ref{fig:metaj}のようになっている．
